...
## Maintained local Flux distribution — 2026-09-16

Release artifacts must include `packages/livewire/flux-pro/` and its adjacent provenance/checksum manifests before Composer install. The local 0.1.1 path package uses `symlink: false`, ordinary discovery and a physical vendor copy; no official Flux repository access is needed for this path install. The original root `flux-pro/` remains available until the final clean-install and rollback gates. Clean Composer/npm installation, production build, package discovery and config/route/view caches are verified in an owned temporary copy. The original-folder deletion and complete all-family adoption remain separate gates. Runtime PHP requests need neither Node nor Sass. After changing maintained package sources, reinstall the path package normally and rebuild frontend assets; do not patch the physical vendor mirror.
