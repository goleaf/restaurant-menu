...
| Onboarding | restaurant_onboardings, restaurant_onboarding_service_points | multiple actor-owned attempts may share organization/brand; branch_id remains unique per restaurant; creation_key is unique with actor-bound replay and payload hash; setup_version rejects stale changes; expected table count plus ordered service-point links detect missing/corrupt sets; links are unique by checkpoint+position and by service point; `completed_at` is the explicit terminal state |
