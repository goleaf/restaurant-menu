<div class="grid min-w-0 gap-5" data-section="catalog-transfer" x-data="catalogTransfer" x-on:catalog-transfer-feedback.window="focusFeedback()">
    ...
    <section class="grid min-w-0 gap-4 rounded-card border border-border-subtle bg-surface p-4 sm:p-5">
        ...
        <details class="min-w-0 rounded-control border border-border-subtle p-3">
            <summary class="touch-target cursor-pointer text-sm font-medium text-text-primary">{{ __('menu.csv.category_reference') }}</summary>
            <p class="mt-2 text-xs text-text-muted">{{ __('menu.csv.reference_help') }}</p>
            <dl class="mt-3 grid gap-x-5 gap-y-2 sm:grid-cols-2">
                @forelse ($categories as $category)
                    <div class="flex min-w-0 gap-3 text-sm"><dt class="shrink-0 font-mono tabular-nums text-text-muted">{{ $category['id'] }}</dt><dd class="break-words text-text-primary">{{ $category['name'] }}</dd></div>
                @empty
    ...
    @if ($previewRows !== [])
        <section class="grid min-w-0 gap-4 rounded-card border border-border-subtle bg-surface p-4 sm:p-5" aria-label="{{ __('menu.csv.preview_title') }}">
            ...
            <ul class="max-h-96 divide-y divide-border-subtle overflow-y-auto rounded-control border border-border-subtle">
                @forelse ($previewRows as $row)
                    <li wire:key="csv-row-{{ $row['line'] }}" class="grid min-w-0 gap-1 p-3 sm:grid-cols-[5rem_minmax(0,1fr)_auto] sm:items-center sm:gap-3">
                        ...
                        <details class="min-w-0">
                            <summary class="touch-target cursor-pointer break-words text-sm font-medium text-text-primary">{{ $row['name'] }}</summary>
                            ...
                            <dl class="mt-3 grid min-w-0 gap-3">
                                @forelse ($row['translations'] as $locale => $translation)
                                    <div class="min-w-0">
                                        ...
                                        <dd class="break-words text-sm text-text-primary">{{ $translation['name'] }}</dd>
                        ...
                        <span class="font-mono text-sm tabular-nums text-text-primary">{{ $row['price'] }}</span>
                    </li>
                @empty
                @endforelse
            </ul>
    ...
    <section class="grid gap-3 rounded-card border border-border-subtle bg-surface-muted p-4 sm:p-5">
