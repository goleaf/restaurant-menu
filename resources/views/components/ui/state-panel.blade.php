<section
    ...
    {{ $attributes->class(['rounded-card border px-4 py-3.5 sm:px-5 sm:py-4', $toneClasses]) }}
>
    <div class="flex items-start gap-3">
        <span class="flex size-9 shrink-0 items-center justify-center rounded-control border border-current/20 bg-surface text-current">
            <flux:icon :name="$icon" variant="mini" @class(['size-5', 'motion-safe:animate-spin' => $busy]) />
        </span>

        <div class="min-w-0 flex-1">
            <p class="content-safe text-sm font-semibold leading-5 text-current">{{ __($title) }}</p>
