<section data-page="{{ $dataPage }}" wire:poll.visible.1s="refreshDepartment" class="flex h-full w-full flex-1 flex-col gap-6">
    <header class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div class="min-w-0">
            ...
            <h1 class="mt-1 text-2xl font-semibold text-zinc-950 dark:text-white">{{ $pageTitle }}</h1>
    ...
    <div class="grid gap-4 xl:grid-cols-2">
        @forelse ($tickets as $ticket)
            <article wire:key="{{ $dataPage }}-ticket-{{ $ticket['id'] }}" class="overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
                <header class="border-b border-zinc-200 px-4 py-4 dark:border-zinc-800">
                    <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                        ...
                        <div class="text-sm text-zinc-500 dark:text-zinc-400 md:text-end">
                            <p>{{ __('Created') }}: {{ $ticket['sent_at'] ?? __('time not set') }}</p>
                            <p>{{ __('Timer') }}: {{ $ticket['elapsed_label'] }}</p>
                            <p>{{ $itemCountLabel }}: {{ $ticket['item_count'] }}</p>
                        </div>
                    </div>
                </header>
                ...
                <div class="divide-y divide-zinc-200 dark:divide-zinc-800">
                    @foreach ($ticket['items'] as $item)
                        <section wire:key="{{ $dataPage }}-ticket-item-{{ $item['id'] }}" class="px-4 py-4">
                            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                                <div class="min-w-0">
                                    <div class="flex flex-wrap items-center gap-2">
                            ...
                            <div class="mt-4 grid grid-cols-3 gap-2">
                                @foreach ($statusOptions as $statusValue => $statusLabel)
                                    <button
                                        wire:key="{{ $dataPage }}-ticket-item-{{ $item['id'] }}-status-{{ $statusValue }}"
                                        type="button"
