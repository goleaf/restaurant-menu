...
The accepted `ui-flux-pro-001` extension now installs the local Flux Pro 0.1.0 adaptation with Free 2.17.0. It introduces no platform extension, runtime service, database change or runtime account credential. The internal Composer path is part of the application release. Track interaction/workflow acceptance and clean-install/rollback proof in [`IMPLEMENTATION_PLAN.md`](IMPLEMENTATION_PLAN.md); remove the original directory only after those gates.