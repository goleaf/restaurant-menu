...
class StaffFilterForm extends Form
{
    ...
    public function validatedFilters(string $section): array
    {
        $values = $this->validate([
            ...
            'status' => ['nullable', 'string', Rule::in($section === 'invitations' ? ['pending', 'accepted', 'cancelled', 'expired', 'rejected'] : ['active', 'invited', 'suspended', 'removed'])],
            'sort' => ['required', 'string', Rule::in(['newest', 'oldest', 'name'])],
        ]);
