<section
    ...
    wire:poll.visible.2s="refreshNotifications"
    class="rounded-lg border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900"
>
    <div class="flex items-start justify-between gap-3">
        <div class="min-w-0">
            <p class="text-xs font-medium uppercase text-emerald-700 dark:text-emerald-300">{{ __('Уведомления') }}</p>
            <h2 class="mt-1 text-lg font-semibold leading-tight text-zinc-950 dark:text-white">{{ __('Что происходит за столом') }}</h2>
        </div>
    ...
    @if (! $canRead)
        <p class="mt-3 rounded-lg bg-zinc-50 px-3 py-2 text-sm text-zinc-600 dark:bg-zinc-950 dark:text-zinc-300">
            {{ __('Уведомления доступны активным гостям за этим столом.') }}
        </p>
    @elseif ($unreadCount === 0)
        <p class="mt-3 rounded-lg bg-zinc-50 px-3 py-2 text-sm text-zinc-600 dark:bg-zinc-950 dark:text-zinc-300">
            {{ __('Новых уведомлений нет.') }}
        </p>
    @else
                ...
                >
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <p class="mt-1 text-sm leading-5 text-zinc-700 dark:text-zinc-200">{{ $notification['body'] }}</p>
