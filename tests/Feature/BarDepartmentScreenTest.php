...
test('bartender sees only bar department drinks and updates item status', function () {
    ...
    $component = Livewire::actingAs($bartender)
        ...
        ->assertSee('data-preparation-item="'.$barItem->id.'"', false)
        ...
        ->assertSee('Prompt 62 Table')
        ->assertSee('Prompt 62 Lounge')
        ->assertSee('Milk: Oat')
        ->assertSee('No ice')
        ->assertSee('T-62')
        ->assertSee(__('preparation.timer.received'))
        ...
        ->call('setItemStatus', $barItem->id, KitchenTicketItemStatus::Accepted->value)
        ->assertHasNoErrors()
