<main data-page="branch-bulk-qr-print" class="qr-print-page flex min-h-svh flex-col items-center gap-4 px-4 py-6 qr-print-page-bulk">
    <div class="qr-print-toolbar rm-print-toolbar">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <flux:button icon="arrow-down-tray" type="button" wire:click="downloadPdf" wire:loading.attr="disabled" wire:target="downloadPdf" :disabled="count($printItems) === 0">
                {{ __('qr.actions.download_pdf') }}
            </flux:button>
            <flux:error name="pdf.service_points" />
            <flux:error name="service_points" />
        </div>
    </div>
                ...
                >
                    <div class="qr-sticker-brand">
                        @if ($restaurantLogoUrl)
                            <img src="{{ $restaurantLogoUrl }}" alt="{{ $item['brand_name'] }}" width="208" height="48" class="qr-sticker-logo">
                        @else
                    ...
                    <img src="{{ $item['qr_image_data_uri'] }}" alt="{{ __('qr.labels.image') }}" width="720" height="720" class="qr-sticker-image">
