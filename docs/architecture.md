...
## Personal format boundary — 2026-09-20

Settings/Profile mounts a separate class-based DisplayFormats component. Its Livewire Form validates raw transport against finite presets; UpdateUserDisplayFormatsAction writes only the authenticated actor's preference columns. DisplayPreferences is the shared immutable presentation boundary for dates, numeric values, money, examples and bounded cache fingerprints; Blade receives labels and examples, never queries. Public guest consumers explicitly use language defaults. Deferred staff report callbacks capture preferences alongside locale, including last-success keys. Native input values and machine exports are unchanged. No route, dependency, service process or new authorization target is introduced.


## Prompt 10 preparation workspace

`restaurant.preparation.dashboard` is the canonical class-based Departments Dashboard. Kitchen/Bar remain thin exact-family entry wrappers sharing one view. The existing restaurant context resolves branch/department/ticket conflicts explicitly; URL state never becomes authority. Preparation access unions kitchen IDs and bar IDs only after independent existing role/permission/branch decisions; it introduces no observer permission or role seed change.

PreparationFilterForm validates context; PreparationSelectionForm bounds24 unique visible rows. Locked review snapshots freeze the displayed targets; Actions still reload and authorize the current actor, branch, ticket, order and visit. Row changes compare shown status/raw version and served-null, retain the existing transition graph and record one actor-bearing event. Batch operations return explicit per-row results. Ready does not serve, settle, close or redispatch.

Queue read transactions return24 tickets and at most100 rows per ticket/detail page with complete relational counts, portion sums and event timestamps. Selected ticket details bypass only the display-status filter. Order synchronization uses full aggregates without the old1000-row truncation and considers all orders of the visit at its current serving point. Paid/closed/cancelled states remain protected. Print prepares the full saved ticket and reauthorizes every request; private guest names/prices are omitted from preparation presentation.

Post-commit notification delivery uses the existing status-log event and database-only notification transport. Pending/delivered metadata and all notification inserts commit together in a separate bounded transaction; explicit retry rechecks current resource state. No new table, required worker, scheduler, WebSocket or external exactly-once guarantee is introduced. Department deactivation rejects unfinished/ready-unserved work; deletion/type change preserves ticket history. Legacy inactive work remains scoped and reachable; routing diagnostics do not reassign it.


## Prompt 5 floor composition

`ServicePoints/Index` owns validated room/filter/mobile-region/selection state; AreaEditor, PointEditor, BulkCreate, SelectionOperations, QrPanel and PrintPanel retain focused Forms and Actions. `FloorWorkspaceQuery`, AreaNodeQueryService and ServicePointQueryService prepare bounded projections. Area parent selection has its own pagination name; selected room context is pinned independently of lifecycle/search filters. Blade receives prepared paths, localized types, hierarchy validity and the selected-room label.

The orphan areas/area-node-row templates and their depth-only Sass module are removed. Existing Areas and QR legacy entry classes remain scoped redirect adapters, not second editors. `PublicQrUrl` derives a public address from validated app.url plus the relative named route; it does not trust the request Host. QR file integrity/publication and immutable prepared-print storage remain separate from SQLite record creation. No route/controller/schema or global context owner is added.
...
`AuthRequestAdapter` constructs a separate request containing validated credentials for the installed Fortify pipeline and contracts. It preserves the configured guard/provider, normalization, pipeline callback, named throttling and session preparation, without overwriting the global Livewire request. Password reset uses Laravel's broker and existing password-reset implementation. Sensitive form fields are cleared before dehydration, including settings errors and unrelated refreshes. MFA setup data and displayed recovery codes are render-only, never signed public state. Feature-enabled passkey entry uses the installed vendor ceremony and completes with full navigation. Vendor auth POST, signed email verification and WebAuthn transports remain framework infrastructure, not another first-party form interface.
...
Token-bearing password-reset GET uses Fortify's own view contract only to establish a session-bound attempt and redirect to the tokenless Livewire form. Original route names used by mail remain. The native session middleware subclass excludes credential-entry routes from previous-URL history while preserving Laravel session locking and its lazy cache resolver. Previous report GETs open `Exports\Index` with checked route branch/export and period; they no longer generate a file on navigation. Previous backup GETs redirect to the existing protected platform panel. Obsolete application mutation POSTs are removed and fail safely; no automatic replay or GET mutation replaces them.
...
Successful SQLite replacement also revokes active MCP credentials from the restored snapshot within the existing transaction, preventing resurrection of a token revoked after the backup. Already revoked timestamps and rollback semantics are preserved.
...
Named account, restaurant-management and platform routes own their mode before any incidental branch/department query is considered. They cannot be relabelled as a restaurant workspace by a query string. Concrete object and nested restaurant routes still validate their whole ownership chain and reject contradictory identifiers. Table and ticket return links use that verified object context, including the department, rather than a mutable last-restaurant preference.
...
The existing workspace navigation owner guards pending saves before Livewire changes history snapshots. It uses native Navigation API indices where available; the fallback tracks actual History API writes and preserves Livewire state, with ownership-safe cleanup. It neither owns a second router nor replays a denied mutation or transition. The exact older-browser/unknown-segment limitation and its native-departure behavior are documented in DECISIONS.md; it is not presented as universal cancellation support.
...
Prompt14 replaces the compound settings write. `BranchPublicProfileForm` and the guest/settlement/locale/advanced Forms own independent validation; `UpdateBranchPublicProfileAction`, `SaveBranchSettingsGroupAction` and `SaveBranchMediaAction` own exact writes. `ExecuteBranchSettingsChangeAction` reauthorizes current actor/tenant/archive state and records UUID-bound results inside a short SQLite IMMEDIATE transaction. Fingerprints cover only the relevant group and direct dependencies; the currency guard rechecks monetary data in that transaction. `BranchSettingsQueryService::effective` never inserts on read. Media processing precedes the transaction; replacement rollback compensation and commit-only deletion retain source files. Availability remains independently versioned. Legacy compound Action stubs fail closed and have no component callers.
...
| Restaurant onboarding | explicit private actor-owned restaurant preparation attempts, four-group state derivation and atomic creation through existing domain Actions | `RestaurantOnboarding`, onboarding Actions, `RestaurantSetupQueryService`, `Livewire/Onboarding` |
...
- Restaurant preparation uses explicit private attempt identity and one concrete restaurant; multiple attempts per actor can share existing parents. Actor/request-key/payload-hash replay protection coordinates atomic organization/brand/restaurant creation through existing Actions. Each mutation reloads the actor, tenant chain, capability and subscription. Group selection is presentation state; actual scoped relationships determine saved results, menu and room preparation are independent, and completed_at remains write-once history. Current readiness uses BranchReadinessService on the selected restaurant, never a first-user-attempt lookup or expensive per-row analysis. Existing content, permanent QR identities and operational availability survive resume; archive/restore and publication require separate explicit domain operations.
...
The selected Main projection can omit full availability relations; the existing explicit preview evaluates configuration using the same authoritative services. No empty fabricated relation or second availability calculation substitutes for an unchecked result. Prepared preview freshness is locked UI state invalidated by relevant input, main/media writes and child resource events; it is independent of Livewire transport dirty state.
