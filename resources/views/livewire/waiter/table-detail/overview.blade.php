<div class="contents">
    ...
    <section class="rounded-card border border-border-subtle bg-surface p-4">
        ...
        <dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
            <div>
                <dt class="text-text-muted">{{ __('guest.table.branch') }}</dt>
                <dd class="mt-1 font-medium text-text-primary">{{ data_get($overview, 'branch.name') }}</dd>
            </div>
            <div>
                <dt class="text-text-muted">{{ __('guest.table.zone') }}</dt>
                <dd class="mt-1 font-medium text-text-primary">{{ data_get($overview, 'zone.name') ?? __('qr.filters.no_zone') }}</dd>
            </div>
            <div>
                <dt class="text-text-muted">{{ __('guest.table.service_point') }}</dt>
                <dd class="mt-1 font-medium text-text-primary">{{ data_get($overview, 'service_point.name') }}</dd>
            </div>
            <div>
            ...
            <div>
                <dt class="text-text-muted">{{ __('ui.waiter.dashboard.opened') }}</dt>
                <dd class="mt-1 font-medium text-text-primary">{{ data_get($overview, 'session.started_at') ?? __('ui.departments.dashboard.time_not_set') }}</dd>
            </div>
            <div>
                <dt class="text-text-muted">{{ __('ui.waiter.table_detail.opened_by') }}</dt>
                <dd class="mt-1 font-medium text-text-primary">{{ data_get($overview, 'session.opened_by') ?? __('qr.labels.not_set') }}</dd>
            </div>
            @if (data_get($overview, 'confirmed_order_count', 0) > 0)
                <div>
                    <dt class="text-text-muted">{{ __('ui.waiter.table_detail.confirmed_orders') }}</dt>
                ...
                <div>
                    <dt class="text-text-muted">{{ __('ui.waiter.table_detail.current_draft_total') }}</dt>
                    <dd class="mt-1 font-medium text-text-primary">{{ data_get($overview, 'current_draft_total') }}</dd>
                </div>
            @endif
        </dl>
        ...
        @error('table_session')
            <p class="mt-5 rounded-md bg-danger-surface px-3 py-2 text-sm font-medium text-danger">{{ $message }}</p>
        @enderror

        <div class="mt-5 grid gap-4 lg:grid-cols-2">
            @if (data_get($overview, 'merge.can_merge'))
                <div id="merge-table" class="rounded-lg border border-border-subtle p-4">
                    <h3 class="text-sm font-semibold text-text-primary">{{ __('ui.waiter.table_detail.obieedinit_stoly') }}</h3>
                    <p class="mt-2 text-sm text-text-muted">{{ __('ui.waiter.table_detail.dobavte_eshhe_odno_svobodnoe_mesto_k_etoi_sessii_kaz') }}</p>

                    @if ($mergeFeedbackMessage)
                        <p class="mt-3 rounded-md bg-success-surface px-3 py-2 text-sm font-medium text-success">{{ $mergeFeedbackMessage }}</p>
                    @endif
                    ...
                    @if (data_get($overview, 'merge.available_service_points') !== [])
                        <div class="mt-3 space-y-3">
                            ...
                            <flux:button icon="plus" variant="primary" type="button" class="w-full" wire:click="mergeServicePoint" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:target="mergeServicePoint">
                    ...
                    @else
                        <p class="mt-3 rounded-md bg-surface-muted px-3 py-2 text-sm text-text-muted">{{ __('ui.waiter.table_detail.svobodnyx_mest_dlia_obieedineniia_seicas_net') }}</p>
                    @endif
                </div>
            @endif

            @if (data_get($overview, 'transfer.can_transfer'))
                <div id="transfer-table" class="rounded-lg border border-border-subtle p-4">
                    <h3 class="text-sm font-semibold text-text-primary">{{ __('ui.waiter.table_detail.perenesti_stol') }}</h3>
                    <p class="mt-2 text-sm text-text-muted">{{ __('ui.waiter.table_detail.vyberite_svobodnoe_mesto_zakazy_i_gosti_ostanutsia_v') }}</p>

                    @if ($transferFeedbackMessage)
                        <p class="mt-3 rounded-md bg-success-surface px-3 py-2 text-sm font-medium text-success">{{ $transferFeedbackMessage }}</p>
                    @endif
                    ...
                    @if (data_get($overview, 'transfer.available_service_points') !== [])
                        <div class="mt-3 space-y-3">
                            ...
                            <flux:button icon="arrow-right" variant="primary" type="button" class="w-full" wire:click="transferTableSession" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:target="transferTableSession">
                    ...
                    @else
                        <p class="mt-3 rounded-md bg-surface-muted px-3 py-2 text-sm text-text-muted">{{ __('ui.waiter.table_detail.svobodnyx_mest_dlia_perenosa_seicas_net') }}</p>
                    @endif
                </div>
            @endif
        </div>
