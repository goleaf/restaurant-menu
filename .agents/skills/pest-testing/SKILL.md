...
## Multipart browser fixture boundary

The installed Pest browser HTTP driver omits multipart files when building the Laravel request. ProductMenuWorkflowTest uses a test-only, bounded parser for allowlisted PNG fixture requests and initializes Livewire's temporary test disk. Keep this adapter isolated from runtime, send real image bytes through normal validation, decode Livewire's `livewire-files:` transport only in assertions, and verify exact persisted count/identity rather than treating upload-finish events or existing seeded photos as proof of an upload.
