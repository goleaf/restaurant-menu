...
var hideReference = (options = {}) => ({
  name: "hide",
  options,
  async fn(state) {
    let { rects } = state;
    let { strategy = "referenceHidden", ...detectOverflowOptions } = evaluate(options, state);
    switch (strategy) {
      case "referenceHidden": {
        let overflow = await detectOverflow(state, { ...detectOverflowOptions, elementContext: "reference" });
        let offsets = getSideOffsets(overflow, rects.reference);
        return { data: { referenceHiddenOffsets: offsets, referenceHidden: isAnySideFullyClipped(offsets) } };
      }
      case "escaped": {
        let overflow = await detectOverflow(state, { ...detectOverflowOptions, altBoundary: true });
  ...
  return (event, forceX, forceY) => {
    computePosition2(invoke, target, {
      ...
      middleware: [
        ...
        ...onReferenceHidden ? [hideReference({ strategy: "referenceHidden" })] : []
      ]
...
var UIDisclosure = class extends UIElement {
  boot() {
    ...
    let collapses = details.getAttributeNames().some((name) => name === "x-collapse" || name.startsWith("x-collapse."));
    let refresh = (initial = false) => {
      if (details._disclosable.getState()) {
        ...
        if (!collapses || initial) setAttribute2(details, "hidden", "until-found");
      }
    };
    ...
    if (collapses) {
      on(details, "transitionend", (e) => {
        if (e.target !== details || e.propertyName !== "height" || details._disclosable.getState()) return;
        setAttribute2(details, "hidden", "until-found");
      });
    }
    on(details, "beforematch", () => details._disclosable.setState(true));
    on(button, "click", (e) => {
      if (!this.disabled) {
...
var UIDropdown = class extends UIElement {
  boot() {
    ...
    let tooltip = this.hasAttribute("data-flux-tooltip");
    if (!trigger) {
...
function initializeNavmenu(overlay) {
  if (!overlay.matches("[data-flux-navmenu]")) return;
  let closeOnSelection = (target) => {
    if (target.closest?.("[data-flux-navmenu-item]")) overlay._popoverable.hide();
  };
  on(overlay, "mouseup", (e) => closeOnSelection(e.target));
  on(overlay, "click", (e) => !e.isTrusted && closeOnSelection(e.target));
}
...
var UISidebar = class extends UIElement {
  boot() {
    ...
    document.addEventListener("flux-sidebar-toggle", toggle);
    this.onUnmount(() => document.removeEventListener("flux-sidebar-toggle", toggle));
    this.addEventListener("click", (e) => {
    ...
    let dropdownObserver = new MutationObserver((mutations) => {
      if (mutations.some((mutation) => mutation.target.matches?.("[data-flux-sidebar-group-dropdown]"))) {
        this.refreshActiveState();
      }
    });
    dropdownObserver.observe(this, { attributes: true, attributeFilter: ["data-open"], subtree: true });
    this.onUnmount(() => dropdownObserver.disconnect());
  }
  refreshActiveState() {
...
var UIToast = class extends UIElement {
  mount() {
    if (!this.closest("ui-toast-group")) {
      ...
      this.onUnmount(() => document.removeEventListener("keydown", onEscape));
  ...
  hydrateLinkTemplate(template, link) {
    ...
    let anchor = linkTemplate.content.cloneNode(true).firstElementChild;
    Object.entries(link).forEach(([key, value3]) => {
      if ([null, void 0, false].includes(value3) || key === "text") return;
      if (key === "navigate" && value3) return anchor.setAttribute("wire:navigate", "");
      if (["href", "target", "rel", "download"].includes(key)) {
        if (key === "download" && value3 === true) return anchor.setAttribute(key, "");
        anchor.setAttribute(key, value3);
      }
    });
    anchor.querySelectorAll('slot[name="text"]').forEach((slot) => slot.replaceWith(document.createTextNode(link.text || "")));
    linkTemplate.replaceWith(anchor);
  }
};
  ...
  mount() {
    ...
    for (let i = 0; i < this.length; i++) {
      ...
      on(this.inputEls[i], "beforeinput", (e) => {
        ...
        insertedCharacter = e.inputType === "insertText" && e.data?.length === 1 ? e.data : null;
      });
      on(this.inputEls[i], "input", (e) => {
        e.stopPropagation();
        let value3 = e.target.value;
        if (value3.length > 1 && insertedCharacter) {
          value3 = e.target.value = insertedCharacter;
        }
        if (value3.length > 1) {
          this.state.setValue(value3);
          this.focusIndex(this.nextIndex());
