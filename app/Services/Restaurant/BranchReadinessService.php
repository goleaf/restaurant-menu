 ...
 * @phpstan-type OrderingState array{kind: string, is_open: bool, can_accept_orders: bool, label: string, detail: string, reason: string|null, until: string|null, until_input: string, closed_until_label: string|null, can_manage: bool, is_temporarily_closed: bool, next_opens_at: string|null, timezone: string}
 */
final class BranchReadinessService
{
    public function __construct(
        private readonly GetBranchOpeningStatusAction $openingStatus,
        private readonly GetMenuAvailabilityStatusAction $menuAvailability,
    ...
    public function handle(User $user, Branch $branch): array
    {
        $branch = Branch::query()->select([
            ...
            'temporary_closed_reason', 'temporary_closed_until', 'deleted_at', 'logo_path',
        ...
        $opening = $this->openingStatus->handle($branch);
        $until = $branch->temporaryClosedUntilForBranch();
        $paused = $branch->is_temporarily_closed && ($until === null || $until->isFuture());
        $menu = $this->menuState($branch);
        ...
        $items = [
            ...
            $this->item('menu', $menu['available'] ? 'ready' : 'blocker', 'readiness.menu', 'readiness.menu_description', $menuUrl),
            $this->item('routing', $menu['routing_ready'] ? 'ready' : 'warning', 'readiness.routing', 'readiness.routing_description', $menuUrl === null ? null : route('organizations.brands.branches.menu.index', [...$routes, 'section' => 'departments'])),
        ...
        return [
            ...
            'ordering' => [
                'kind' => match (true) {
                    $paused => 'manual_pause', $blocked => 'setup_problem', ! $opening['can_accept_orders'] => 'schedule_closed', default => 'open'
                },
                'is_open' => $opening['is_open'] && ! $blocked,
                'can_accept_orders' => $opening['can_accept_orders'] && ! $blocked,
                'label' => $blocked && ! $paused ? __('dashboard.control.setup_problem') : ($opening['can_accept_orders'] ? __('dashboard.control.accepting_orders') : $opening['label']),
                'detail' => $blocked && ! $paused ? __('dashboard.control.setup_problem_help') : $opening['detail'],
                ...
                'next_opens_at' => $opening['next_opens_at'], 'timezone' => $opening['timezone'],
            ],
        ];
    }

    /** @return array{available: bool, routing_ready: bool} */
    private function menuState(Branch $branch): array
    {
        $menus = Menu::query()->select(['id', 'branch_id', 'name', 'status'])
            ->where('branch_id', $branch->id)->where('status', MenuStatus::Active->value)
            ->with(['availabilitySchedules' => fn ($query) => $query->select(['id', 'menu_id', 'day_of_week', 'starts_at', 'ends_at'])])
            ->withExists([
                'items as has_guest_items' => fn ($items) => $this->guestItems($items),
                'items as has_unrouted_items' => fn ($items) => $this->guestItems($items)->whereDoesntHave('kitchenDepartment', fn ($department) => $department->where('branch_id', $branch->id)->where('is_active', true)),
            ])->lazyById(100);
        $available = false;
        $unrouted = false;
        foreach ($menus as $menu) {
            $menu->setRelation('branch', $branch);
            if (! $this->menuAvailability->handle($menu)['is_available']) {
                continue;
            }
            $available = $available || (bool) $menu->getAttribute('has_guest_items');
            $unrouted = $unrouted || (bool) $menu->getAttribute('has_unrouted_items');
        }

        return ['available' => $available, 'routing_ready' => $available && ! $unrouted];
    }

    /** @param Builder<MenuItem> $query @return Builder<MenuItem> */
    private function guestItems(Builder $query): Builder
    {
        return $query->where('is_available', true)
            ->where(fn ($hidden) => $hidden->whereNull('hidden_until')->orWhere('hidden_until', '<=', now()))
            ->whereHas('category', fn ($category) => $category->where('is_active', true)->whereColumn('menu_categories.menu_id', 'menu_items.menu_id'))
            ->where(fn ($variants) => $variants->whereDoesntHave('variants')->orWhereHas('variants', fn ($variant) => $variant->where('is_available', true)));
    }
