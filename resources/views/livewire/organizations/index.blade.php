<section data-page="organizations" class="flex h-full w-full flex-1 flex-col gap-6">
    <x-ui.page-header title="navigation.organizations" eyebrow="ui.organizations.index.administration" icon="building-office-2" />
    ...
    <div class="overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <x-structure.list-toolbar heading="ui.organizations.index.my_organizations" id-prefix="organizations" :visible-count="$visibleCount" :search="$search" />
        ...
        <div class="divide-y divide-zinc-200 dark:divide-zinc-800">
                    ...
                    @else
                            ...
                            @if ($organization['is_owner'] && ! $organization['is_archived'])
                                <form wire:submit="saveLogo({{ $organization['id'] }})" class="mt-3 flex flex-col gap-2 sm:flex-row sm:items-start">
                                    ...
                                    <div class="flex flex-wrap gap-2">
                                        ...
                                        @if ($organization['logo_url'])
                                            <flux:button icon="trash" type="button" variant="primary" color="red" wire:click="removeLogo({{ $organization['id'] }})" wire:loading.attr="disabled" wire:target="removeLogo({{ $organization['id'] }})" class="rm-action-danger">
                        ...
                        @elseif ($organization['is_owner'])
                            <div class="flex flex-wrap gap-2 md:justify-end">
                                ...
                                <flux:button icon="trash" type="button" variant="primary" color="red" wire:click="confirmDelete({{ $organization['id'] }})" class="rm-action-danger">
            ...
            @empty
                <div class="p-4" data-structure-empty="{{ $search !== '' ? 'search' : 'collection' }}">
                    <x-ui.empty-state :heading="$search !== '' ? 'ui.empty.no_results' : ($lifecycle === 'archived' ? 'structure.empty.archived' : 'ui.empty.no_organizations')" :icon="$search !== '' ? 'magnifying-glass' : 'inbox'">
                        @if ($search !== '')
                            <x-slot:actions>
        ...
        @if ($organizationsPaginator->hasPages())
            <div class="border-t border-zinc-200 px-4 py-3 dark:border-zinc-800">
                {{ $organizationsPaginator->links() }}
            </div>
        @endif
    </div>
</section>
