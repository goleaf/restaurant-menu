...
class PrepareBranchCsvExportAction
{
    public function __construct(
        private readonly ResolveExportAccessibleBranchIdsAction $resolveExportAccessibleBranchIds,
        private readonly PreparedDownloadStore $downloads,
    ) {}

    public function handle(User $user, Branch $branch, DataExportType $type, ?CarbonInterface $startedAt = null, ?CarbonInterface $endedAt = null): string
    {
        abort_unless($this->resolveExportAccessibleBranchIds->canExport($user, $branch), 403);
        $disk = Storage::disk('local');
        $disk->makeDirectory('exports');
        $path = $disk->path('exports/'.Str::uuid().'.csv');
        ...
        try {
            return $this->downloads->issue($user, $path, 'restaurant-menu-'.$type->filenamePart().'-branch-'.$branch->id.'-'.now()->format('Y-m-d-His').'.csv', 'csv', $branch->id);
        } finally {
            if (is_file($path)) {
                unlink($path);
            }
    ...
    {
        if (fputcsv($handle, array_map($this->spreadsheetCell(...), $row), escape: '') === false) {
