...
test('recipients retain validation handle rotated forms and switch mismatched accounts in isolated browsers', function (): void {
    $this->withVite();
    ...
    $recipient = visit($invitation->inviteLink());
    ...
    $recipient->assertDisabled('form[action$="/invite/register"] button[type="submit"]')->assertSee(__('invitations.offline'));
    ...
    foreach ([[320, 800], [390, 844], [768, 900], [1024, 900], [1440, 1000]] as [$width, $height]) {
        ...
        $recipient->screenshot(false, "team-recipient-{$width}");
    }
    teamRecipientClick($recipient, 'form[action$="/invite/register"] button[type="submit"]');
    ...
    $oldForm = visit($stale->inviteLink());
    ...
    $existing = User::factory()->create(['email' => 'recipient.existing@example.test']);
    $other = User::factory()->create(['email' => 'recipient.other@example.test']);
    $matching = $create->handle($organization, $role, $owner->fresh(), ['email' => $existing->email, 'branch' => $branch]);
    $account = visit(route('login', absolute: false));
    $account->fill('email', $other->email)->fill('password', 'password')->click('@login-button')->assertPathIs('/dashboard');
    $account->navigate($matching->inviteLink())->assertSee(__('invitations.states.email_mismatch_title'));
