<x-ui.form-field :for="$fieldId" :label="$label" :description="$description" :name="$name" :error="$error">
    <textarea
        ...
        {{ $attributes->class([
            'w-full rounded-control border border-border-subtle bg-surface px-3 py-3 text-base text-text-primary shadow-xs transition-[background-color,border-color,color,box-shadow] duration-state ease-product placeholder:text-text-muted focus:border-border-strong focus:outline-hidden focus:ring-2 focus:ring-focus/20 disabled:cursor-not-allowed disabled:bg-surface-muted disabled:text-text-muted motion-reduce:transition-none',
