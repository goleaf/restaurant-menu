...
class BuildKitchenDashboardAction
{
    ...
    {
        $departmentIds = $this->resolveAccessibleDepartmentIds->handle($user);

        if ($departmentIds->isEmpty()) {
            return $this->emptyPayload(false);
        }

        $departments = $this->departments($departmentIds);
    ...
    {
        return $this->resolveAccessibleDepartmentIds->userHasAccess($user);
    }
     ...
     */
    private function departments(Collection $departmentIds): EloquentCollection
    {
    ...
    private function ticketPayload(KitchenTicket $ticket): array
    {
        ...
        return [
            ...
            'sent_at' => $ticket->sent_at?->format('Y-m-d H:i') ?? $ticket->created_at?->format('Y-m-d H:i'),
