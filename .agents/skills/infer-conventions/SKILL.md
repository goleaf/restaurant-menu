...
- Repository authority first. Read `AGENTS.md`, `docs/index.md` and matching `.ai/rules` before inferring conventions. A majority pattern that violates an explicit current rule is a defect to report, not a convention to record. This repository already requires Eloquent-only SQLite, Action writes, class-based Livewire, presentation-only Blade and EN/LT/RU translations; do not infer alternatives from historical examples. Existing user authorization to record scoped rules remains valid and does not require another confirmation.
