    ...
    @else
        <section class="rounded-lg border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-900">
            <div class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                ...
                <flux:badge :color="$qrCode['status_color']">{{ $qrCode['localized_status'] }}</flux:badge>
            </div>

            <div class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <div>
                    <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">{{ __('qr.labels.branch') }}</p>
                    <p class="mt-1 text-base font-semibold text-zinc-950 dark:text-white">{{ $qrCode['branch_name'] }}</p>
                ...
                <div>
                    ...
                    <p class="mt-1 text-base font-semibold text-zinc-950 dark:text-white">{{ $qrCode['organization_name'] }}</p>
                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">{{ $qrCode['brand_name'] }}</p>
                </div>

                <div>
                    <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">{{ __('qr.labels.zone') }}</p>
                    <p class="mt-1 text-base font-semibold text-zinc-950 dark:text-white">{{ $qrCode['zone_name'] }}</p>
                </div>

                <div>
                    <p class="text-xs font-medium uppercase text-zinc-500 dark:text-zinc-400">{{ __('qr.labels.service_point') }}</p>
                    <p class="mt-1 text-base font-semibold text-zinc-950 dark:text-white">{{ $qrCode['service_point_name'] }}</p>
                    <p class="mt-1 text-sm text-zinc-600 dark:text-zinc-300">
                        {{ $qrCode['display_number'] }}
                    </p>
                </div>
            </div>
            ...
            <div class="mt-5 flex flex-wrap gap-2">
                @if ($qrCode['admin_url'])
                    <flux:button icon="qr-code" :href="$qrCode['admin_url']" wire:navigate>
                        {{ __('qr.actions.open') }}
                    </flux:button>
                @endif

                @if ($qrCode['public_url'])
                    <flux:button icon="arrow-top-right-on-square" :href="$qrCode['public_url']" target="_blank" rel="noopener">
