<section wire:poll.visible.1s="refreshDraftReview" class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_22rem]">
    <div class="overflow-hidden rounded-lg border border-border-subtle bg-surface">
        <div class="border-b border-border-subtle px-4 py-3">
            <h2 class="text-base font-semibold text-text-primary">{{ __('ui.waiter.table_detail.guests_and_positions') }}</h2>
            <p class="mt-1 text-sm text-text-muted">{{ __('ui.waiter.table_detail.guests_are_sorted_alphabetically') }}</p>
        </div>
                ...
                <section wire:key="waiter-table-guest-{{ $guestSection['guest_id'] }}" class="px-4 py-4">
                    <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                        <div class="flex flex-wrap items-center gap-2">
                            <x-ui.plain-text :text="$guestSection['guest_name']" class="block text-base font-semibold text-text-primary" :preserve-lines="false" />
                        ...
                        <p class="text-sm font-semibold text-text-primary">{{ $guestSection['total'] }}</p>
                    </div>

                    <div class="mt-3 space-y-3">
                        @forelse ($guestSection['items'] as $item)
                            <article wire:key="waiter-table-item-{{ $item['id'] }}" class="rounded-md bg-surface-muted p-3">
                                <div class="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                                    <div class="min-w-0">
                                        ...
                                        @if ($item['variant_name'])
                                            <p class="mt-1 text-sm font-medium text-success">{{ $item['variant_name'] }}</p>
                                        @endif
                                        <p class="mt-1 text-sm text-text-muted">
                                            {{ __('ui.waiter.table_detail.unit') }}: {{ $item['unit_total_price'] }} · {{ __('ui.waiter.table_detail.line') }}: {{ $item['total_price'] }}
                                        </p>
                                    </div>
                                    <p class="text-sm font-semibold text-text-primary">{{ $item['total_price'] }}</p>
                                </div>
                        ...
                        @empty
                            <p class="rounded-md bg-surface-muted px-3 py-2 text-sm text-text-muted">{{ __('ui.empty.no_orders') }}</p>
                        @endforelse
                    </div>
                </section>
            @empty
                <div class="px-4 py-8 text-sm text-text-muted">{{ __('ui.empty.no_guests') }}</div>
            @endforelse
        </div>
    </div>

    <aside class="rounded-lg border border-border-subtle bg-surface p-4">
        <h2 class="text-base font-semibold text-text-primary">{{ __('guest.statuses.items.waiter_review') }}</h2>

        @if ($reviewFeedbackMessage)
            <p class="mt-3 rounded-md bg-success-surface px-3 py-2 text-sm font-medium text-success">{{ $reviewFeedbackMessage }}</p>
        @endif
        ...
        @if (data_get($draftReview, 'manual_order.can_add'))
            <div class="mt-4 border-t border-border-subtle pt-4">
                ...
                <p class="mt-1 text-sm text-text-muted">{{ __('ui.waiter.table_detail.add_dishes_for_a_guest_who_orders_through_the_waiter') }}</p>
                ...
                <div class="mt-3 space-y-3">
                    ...
                    @if ($addableMenuItems === [])
                        <p class="rounded-md bg-surface-muted px-3 py-2 text-sm text-text-muted">{{ __('menu.empty.no_items') }}</p>
                    @endif
                    ...
                    @if ($addingVariants !== [])
                        ...
                        @error('addingItemVariantId') <p class="text-sm font-medium text-danger">{{ $message }}</p> @enderror
                    @endif

                    @foreach ($addingModifierGroups as $modifierGroup)
                        <fieldset wire:key="waiter-add-modifier-group-{{ $modifierGroup['id'] }}" class="rounded-lg border border-border-subtle p-3">
                    ...
                    <flux:button icon="plus" variant="primary" type="button" class="w-full" wire:click="addDraftItem" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:target="addDraftItem">
        ...
        @if (data_get($draftReview, 'draft.can_confirm'))
            <div class="mt-4 space-y-3">
                <flux:button icon="check" variant="primary" type="button" class="w-full" wire:click="confirmDraft" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:target="confirmDraft">
                ...
                <p class="text-xs leading-5 text-text-muted">{{ __('ui.waiter.table_detail.confirmation_dispatches_atomically') }}</p>
            </div>
        @endif
        ...
        @if (data_get($draftReview, 'draft.rejection_reason'))
            <div class="mt-4 border-t border-border-subtle pt-4">
                <p class="text-xs font-medium uppercase text-danger">{{ __('ui.waiter.table_detail.rejected_reason') }}</p>
                <x-ui.plain-text :text="data_get($draftReview, 'draft.rejection_reason')" class="mt-1 block text-sm leading-5 text-text-primary" />
                @if (data_get($draftReview, 'draft.rejected_by_user_name'))
                    <p class="mt-2 text-xs text-text-muted">{{ __('ui.waiter.table_detail.rejected_by') }}: {{ data_get($draftReview, 'draft.rejected_by_user_name') }}</p>
                @endif
            </div>
        @endif

        @if (data_get($draftReview, 'draft.can_return_to_draft'))
            <flux:button icon="arrow-uturn-left" type="button" class="mt-4 w-full" wire:click="returnRejectedDraftToDraft" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:target="returnRejectedDraftToDraft">
        ...
        @if (data_get($draftReview, 'draft.order_id'))
            <p class="mt-4 border-t border-border-subtle pt-4 text-sm font-medium text-text-primary">
    ...
    @if ($editingItemId !== null)
        <div class="fixed inset-0 z-50 flex items-end justify-center bg-backdrop px-3 py-0 sm:items-center sm:py-6">
            <div class="max-h-[92dvh] w-full max-w-lg overflow-y-auto overscroll-contain rounded-t-dialog bg-surface p-4 shadow-elevated sm:rounded-dialog">
                <div class="flex items-start justify-between gap-3">
                    <div class="min-w-0">
                        <p class="text-xs font-medium uppercase text-success">{{ __('ui.waiter.table_detail.waiter_edit') }}</p>
                        <h3 class="mt-1 text-lg font-semibold leading-tight text-text-primary">{{ $editingItemName }}</h3>
                        <p class="mt-1 text-sm font-semibold text-text-primary">{{ $editingItemTotalLabel }}</p>
                    </div>
                    <flux:button
                        type="button"
                ...
                <div class="mt-4 space-y-4">
                    ...
                    @error('editingQuantity') <p class="text-sm font-medium text-danger">{{ $message }}</p> @enderror
                    ...
                    @if ($editingVariants !== [])
                        ...
                        @error('editingItemVariantId') <p class="text-sm font-medium text-danger">{{ $message }}</p> @enderror
                    @endif

                    @foreach ($editingModifierGroups as $modifierGroup)
                        <fieldset wire:key="waiter-edit-modifier-group-{{ $modifierGroup['id'] }}" class="rounded-lg border border-border-subtle p-3">
                            <legend class="px-1 text-sm font-semibold text-text-primary">{{ $modifierGroup['name'] }}</legend>
                            <div class="mt-3 grid gap-2">
                                @foreach ($modifierGroup['options'] as $modifierOption)
                                    <flux:button
                ...
                <div class="sticky bottom-0 -mx-4 mt-5 grid gap-2 border-t border-border-subtle bg-surface px-4 pt-3">
                    <flux:button icon="check" variant="primary" type="button" class="w-full" wire:click="updateDraftItem" wire:offline.attr="disabled" wire:loading.attr="disabled" wire:target="updateDraftItem">
