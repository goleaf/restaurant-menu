...
## Prompt 6 refresh — selected dish lifecycle (2026-09-18)

The existing four-section card and common navigation guard remain canonical. An allowed same-restaurant menu/category change refreshes the retained child components' verified parent context without clearing their forms or replacing conflict revisions. Original legacy EN text is prefilled only when its row is absent. Independent photos, variants and shared groups keep their separate save confirmations.

Main shows direct enabled/disabled/temporary-hidden state and explicitly offers the existing guest preview for full configuration availability. It does not hydrate every variant/option on each description synchronization. Prepared preview results carry a separate stale state after relevant input or child writes; the warning persists after transport synchronization and old prices are hidden until explicit refresh. No new global handler, layout, theme or browser router is introduced. Current visual and complete-gate evidence belongs to PROGRESS.md.
