...
test('requirements traceability covers every canonical requirement with consistent evidence', function (): void {
    ...
    preg_match_all('/^\| `(?<id>[a-z0-9-]+)` \|.*\| (?<status>[^|]+) \|$/m', File::get(base_path('docs/compliance-matrix.md')), $complianceMatches, PREG_SET_ORDER);
    ...
    expect($canonical)->toHaveCount(53)
    ...
    foreach ($traceMatches as $trace) {
        expect(trim($trace['source']))->toMatch('/\[docs\/requirements\.md:\d+\]\(requirements\.md#L\d+\)/')
            ...
            ->and(trim($trace['status']))->toBeIn(['Готово', 'Частично реализовано', 'Не применимо — функции отключены конфигурацией'])
            ->and(trim($trace['evidence']))->not->toBe('')
            ->and(strtolower($trace['evidence']))->not->toMatch('/\b(?:todo|tbd|fixme)\b/');

        expect($compliance->get($trace['id']))->toStartWith(match (trim($trace['status'])) {
            'Готово' => 'implemented and verified',
