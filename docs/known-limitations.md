...
There are no unresolved dependency advisories, migration/seed failures, static-analysis findings or local test failures as of the 2026-08-23 gates. The local PHP 8.5.8 CLI now loads Xdebug 3.5.0 with normal execution disabled by default; `composer test:coverage` explicitly enabled coverage and passed at 90.6%. Physical device, non-Chromium and assistive-technology evidence remains open above.
