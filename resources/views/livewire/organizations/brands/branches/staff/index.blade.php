<section data-page="branch-staff" class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div class="divide-y divide-zinc-200 dark:divide-zinc-800">
                        ...
                        @if ($editingMembershipId === $member['id'])
                            <div class="mt-3 max-w-sm">
                                <flux:select wire:model="editingRoleId" :label="__('staff.role')" required>
                                    @foreach ($roleOptions as $role)
                                        <flux:select.option wire:key="branch-edit-role-{{ $member['id'] }}-{{ $role['id'] }}" value="{{ $role['id'] }}">
                                            {{ $role['label'] }}
                                        </flux:select.option>
                                    @endforeach
                                </flux:select>
                    ...
                    <div class="flex flex-wrap gap-2 md:justify-end">
                        @if ($editingMembershipId === $member['id'])
                            ...
                            <x-dangerous-action-confirmation
                                name="change-branch-staff-role-{{ $member['id'] }}"
            ...
            @empty
                <div class="px-4 py-8 text-sm text-zinc-500 dark:text-zinc-400">
                    {{ $staffSearch !== '' ? __('ui.empty.no_results') : __('staff.empty.no_branch_staff') }}
                </div>
            @endforelse
        </div>
    ...
    <div class="overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
        <div class="flex flex-col gap-3 border-b border-zinc-200 px-4 py-3 sm:flex-row sm:items-end sm:justify-between dark:border-zinc-800">
            ...
            <flux:input wire:model.live.debounce.300ms="invitationSearch" :label="__('layout.search').' — '.__('staff.branch_invitations')" type="search" autocomplete="off" class="sm:max-w-xs" />
        </div>

        <div class="divide-y divide-zinc-200 dark:divide-zinc-800">
            @forelse ($invitationRows as $invitation)
                <div wire:key="branch-invitation-{{ $invitation['id'] }}" class="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="min-w-0">
                        <div class="flex flex-wrap items-center gap-2">
            ...
            @empty
                <div class="px-4 py-8 text-sm text-zinc-500 dark:text-zinc-400">
                    {{ $invitationSearch !== '' ? __('ui.empty.no_results') : __('staff.empty.no_branch_invitations') }}
                </div>
            @endforelse
        </div>
