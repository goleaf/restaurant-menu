...
Latest memory refresh: 2026-06-04 after Prompt 110 service point search filters. Branch public profiles, branch opening hours, temporary branch closed mode, menu availability schedules, multiple active branch menus, branch service modes, bulk service point creation, QR label presets, QR short-code lookup, branch service point search/filter pagination, and waiter-side schedule checks are now part of the baseline branch setup and order-review context.

The memory refresh after Prompt 110 records the current service point search/filter status and keeps the next-step guardrails in `docs/NEXT_STEPS.md`.
