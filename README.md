...
- Waiters with `cancel_orders`, directors, shift managers, and superadmin users can cancel an order.
- Cancellation requires a non-empty reason.
- The order status becomes `cancelled`; `order_status_logs` and `audit_logs` record the actor, status change, reason, and ready/served warning counts.
- Guests see that the order was cancelled and can read the reason in the guest order status block.
- Kitchen/bar dashboards hide cancelled-order tickets, and direct ticket item updates are blocked for cancelled orders.
- If positions are already ready or served, the waiter table detail shows a warning before cancellation.

Focused Prompt 121 command:
...
Prompt 120 keeps payments manual/offline while adding optional service charge and tips to the waiter/cashier bill summary.

- Branch settings now include `service_charge_percent` next to the existing `service_charge_enabled` and `tips_enabled` flags.
- When service charge is enabled, the waiter payment summary adds the configured percentage to confirmed order subtotals.
- When tips are enabled, waiter/cashier staff can enter a manual tips amount while recording table or guest payment.
- Payment history stores snapshots in `manual_payments`: covered subtotal, service charge percent, service charge amount, tips amount, and total paid amount.
- Tips do not reduce the required bill balance; they are recorded on top of the subtotal/service charge.
- No online payment provider, tax logic, Redis, WebSocket, S3, Docker, or paid service is used.

Focused Prompt 120 command:
...
Prompt 119 keeps payment manual and offline, but lets staff settle a table by guest from the waiter table detail screen.

- Each guest balance is calculated from confirmed `order_items`.
- The table bill is the sum of confirmed guest items.
- Cashier/payment staff can mark the whole table paid or mark an individual guest paid.
- Guest payments store `manual_payments.table_session_guest_id`.
- The waiter payment block shows unpaid guests and clears the list when all guest balances are paid.

Focused Prompt 119 command:
...
Prompt 113 lets staff with waiter draft-edit access add dishes from the waiter table detail screen even when the active table has no guest-sent draft yet.

- The waiter can choose an existing active guest or type a new guest name for a classic offline order.
- A new manual guest is stored in `table_session_guests` with local metadata and a non-guessable `guest_token`; it is not a user account.
- The system creates or reuses a waiter-review draft for the active `table_session`, then uses the existing draft item snapshot logic for menu item names, prices, modifiers, comments, and totals.
- The waiter still confirms the draft through the normal confirmation action before it becomes a real `order`; kitchen/bar dispatch remains a separate explicit action.
- Guests already in the session see the updated draft through existing Livewire polling.

No new routes, migrations, infrastructure, Redis, WebSockets, S3, Docker, paid services, or external APIs were added.
...
The same page supports bulk service point creation for simple numbering runs. A manager chooses zone, type, prefix, start number, end number, and capacity. The UI shows a preview such as `T1`, `T2`, `T3` before creating records, marks existing `internal_code` values as skipped, and creates only missing service points. Bulk creation stores the generated label as `name`, `display_number`, and stable `internal_code`.

Bulk creation does not automatically create QR codes. After creating service points, the UI suggests generating QR later through the existing bulk QR print page so staff can review the new places before printing stickers.

The list on the same branch page uses server-side search, filters, and pagination instead of loading every service point at once. The branch filter is the current route branch; staff can narrow by zone, type, status, active/inactive state, and active QR presence, and search by name, display number, stable internal code, or printed QR `short_code`. Existing SQLite indexes cover the branch-scoped hot paths and `qr_codes.short_code` remains unique, so no heavy search service or new infrastructure is used.

The same branch page includes a simple visual floor board. It shows the currently loaded service point page grouped into zone sections, with type icons, status badges, active/disabled and QR badges, and quick actions for opening a table, showing or creating QR, and editing. The board deliberately uses the existing paginated service point query instead of loading the whole branch into memory, so large SQLite branches keep the Prompt 110 pagination guardrail.
...
The single-sticker and branch bulk-print pages support these simple QR label design presets:
...
The presets are CSS-only browser print layouts. They do not create, reissue, disable, or alter QR records, and they do not add PDF services or heavy libraries.
